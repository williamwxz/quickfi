...
contract LoanOrigination is ILoanOrigination, AccessControl, ReentrancyGuard {
    ...
    constructor(
        ...
        require(_tokenRegistry != address(0), "LoanOrigination: Token registry cannot be zero address");
    ...
    function requestLoan(
        ...
        require(stablecoin != address(0), "LoanOrigination: Stablecoin cannot be zero address");

        // Check if stablecoin is supported
        require(ITokenRegistry(tokenRegistry).isTokenSupported(stablecoin), "LoanOrigination: Stablecoin not supported");

        // Get token info
        ITokenRegistry.TokenInfo memory tokenInfo = ITokenRegistry(tokenRegistry).getTokenInfo(stablecoin);

        // Check loan amount limits
        require(principal >= tokenInfo.minLoanAmount, "LoanOrigination: Principal below minimum");
        require(principal <= tokenInfo.maxLoanAmount, "LoanOrigination: Principal above maximum");
        ...
        uint256 currentValuation = ITokenizedPolicy(collateralToken).getValuation(collateralTokenId);
        require(currentValuation > 0, "LoanOrigination: Oracle valuation must be greater than zero");
    ...
    function updateTokenRegistry(address newRegistry) external {
        require(hasRole(ADMIN_ROLE, msg.sender), "LoanOrigination: Must have admin role");
        require(newRegistry != address(0), "LoanOrigination: Registry cannot be zero address");
